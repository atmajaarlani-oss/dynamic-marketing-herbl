...
export default function StatusClient({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    if (order.status === 'paid' && !purchaseTrackedRef.current) {
      ...
      trackEvent('Purchase', {
        ...
        currency: 'IDR',
        content_name: order.nama_produk,
        content_type: 'product',
        num_items: order.jumlah,
      })
    }
  ...
  const waLink = () => {
    const msg = encodeURIComponent(
      `Hai Herbal Insani\nOrder: ${order?.order_id ?? orderId}\nProduk: ${order?.nama_produk ?? '-'} x${order?.jumlah ?? '-'}\nResi: ${order?.resi ?? 'belum ada'}`
    )
      ...
      <div className="mx-auto max-w-lg">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Produk</span><span className="font-medium">{order.nama_produk} x{order.jumlah}</span></div>
